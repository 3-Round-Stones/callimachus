...
echo Java 6 or newer required to run the server
goto end
...
"%JAVA%" -server -mx512m -Dfile.encoding=UTF-8 "-Djava.library.path=%LIB_DIR%" "-Djava.io.tmpdir=%TMP_DIR%" "-Djava.util.logging.config.file=%LOG_CONF%" -cp "%LIB_DIR%\*" %MAIN% -d "%BASE_DIR%" %CMD_LINE_ARGS%
goto end
